...
// *******************************  auctProduct(Admin) ********************* 
	...
	public String productInstAdmin(Product dto, ProductVo vo, Model model, RedirectAttributes redirectAttributes)
		 ...
		 * get방식 return "redirect:/product/productViewAdmin?acprSeq=" + dto.getAcprSeq()
		 * + makeQueryString(vo);
		 */
	 ...
	 * this.naverLoginBO = naverLoginBO; } 네이버로그인콜백 public String
	 * callback(@RequestParam String code, @RequestParam String state, HttpSession
	 * session) throws IOException { OAuth2AccessToken oauthToken =
	 ...
	 * System.out.println("result"+apiResult); session.setAttribute("sessSeq", 0);
	 * //생략 가능
	 * 
